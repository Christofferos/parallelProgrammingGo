...
func Julia(f ComplexFunc, n int) image.Image {
	...
	for inboundsx := bounds.Min.X; inboundsx < bounds.Max.X; inboundsx+=deltabound {
		for inboundsy := bounds.Min.Y; inboundsy < bounds.Max.Y; inboundsy+=deltabound {
			wg.Add(1)
			go func(inboundsx, inboundsy, deltabound int, s float64, wg *sync.WaitGroup, img *image.RGBA) {
				for i := inboundsx; i < inboundsx+deltabound; i++ {
					for j := inboundsy; j < inboundsy+deltabound; j++ {
						n := Iterate(f, complex(float64(i)/s, float64(j)/s), 256)
						r := uint8(0)
						g := uint8(0)
						b := uint8(n % 32 * 8)
						img.Set(i, j, color.RGBA{b, g, r, 255})
					}
				}
				wg.Done()
			}(inboundsx, inboundsy, deltabound, s, wg, img)
		}
	}
	wg.Wait()
